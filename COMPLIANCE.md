...
| 41 | Fare prediction | PARTIAL (synthetic) | `app/ml/` — 3 gradient-boosted quantile regressors (q10/q50/q90), time-based split, MAE/RMSE/coverage; `python -m app.ml.train`; `GET /api/predictions/fare` returns a **range** (lower/point/upper), model version + metrics; `{available: false}` with a reason when no model. Trained on synthetic data → clearly labelled, never fed into the index. Tests: `test_ml_festivals.py`. |
